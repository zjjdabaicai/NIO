...
/*
    一、通道 (Channel) : 用于源节点与目标节点的连接。在 Java NIO 中负责缓冲区中的数据的传输。Channel 本身不存储数据，因此需要缓冲区进行传输。
    ...
    二、通道的主要实现类
     ...
     五、分散 (Scatter) 与聚集 (Gather)
        分散读取(Scattering Reads) : 将通道中的数据分散到多个缓冲去中
...
public class TestChannel {
    ...
    // 使用（直接缓冲区）完成文件的复制（内存映射文件） 只有 ByteBuffer 支持
    @Test
